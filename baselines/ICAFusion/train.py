...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        if not opt.resume:
            # Single-stream YOLOv5 pretraining needs to seed the duplicated infrared branch.
            # Full dual-stream checkpoints already contain both branches and must be loaded as-is.
    ...
    if opt.rect_size:
        imgsz = imgsz_test = [check_img_size(x, gs) for x in opt.rect_size]
    ...
    img_area = imgsz[0] * imgsz[1] if isinstance(imgsz, (list, tuple)) else imgsz ** 2
    hyp['obj'] *= img_area / 640 ** 2 * 3. / nl  # scale to image size and layers
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rect-size', nargs=2, type=int, default=None, help='rectangular image size as height width')
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        if opt.rect_size:
            opt.rect_size = [check_img_size(x, 32) for x in opt.rect_size]
