...
def img_size_max(img_size):
    return max(img_size) if isinstance(img_size, (list, tuple, np.ndarray)) else img_size
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.mosaic_border = [-img_size_max(img_size) // 2, -img_size_max(img_size) // 2]
        self.stride = stride
        self.path = path
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if self.rect:
            ...
            if isinstance(img_size, (list, tuple, np.ndarray)):
                self.batch_shapes_rgb = np.tile(np.array(img_size, dtype=int), (nb_rgb, 1))
            else:
            ...
            if isinstance(img_size, (list, tuple, np.ndarray)):
                self.batch_shapes_ir = np.tile(np.array(img_size, dtype=int), (nb_ir, 1))
            else:
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        r = img_size_max(self.img_size) / max(h0, w0)  # ratio
