...
class DetectionValidator(BaseValidator):
    ...
    def get_stats(self):
        ...
        stats = {
            k: torch.cat(v, 0).cpu().numpy() if len(v) else np.array([]) for k, v in self.stats.items()
        }  # to numpy
        if len(stats) and stats['tp'].any():
            self.metrics.process(**stats)
        self.nt_per_class = (np.bincount(stats['target_cls'].astype(int), minlength=self.nc)
                             if stats['target_cls'].size else np.zeros(self.nc, dtype=int))  # number of targets per class
