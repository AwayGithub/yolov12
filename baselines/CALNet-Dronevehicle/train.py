...
def train_rgb_ir(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    init_seeds(opt.seed + max(RANK, 0))
    ...
    # Optimizer
    accumulate_steps = getattr(opt, 'accumulate', 0)
    nbs = batch_size * accumulate_steps if accumulate_steps else 64  # nominal batch size
    accumulate = accumulate_steps or max(round(nbs / batch_size), 1)  # accumulate loss before optimizing
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                accumulate = accumulate_steps or max(1, np.interp(ni, xi, [1, nbs / batch_size]).round())
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov5l_fusion_transformerx3_resnetcsp_dronevehicle_aligned.yaml', help='model.yaml path')   # BottleneckCSP !!!!!!!!!!!q
    ...
    parser.add_argument('--accumulate', type=int, default=0, help='gradient accumulation steps (0 uses default scaling)')
