...
class LoadMultiModalImagesAndLabels(Dataset):
    ...
    def __init__(self, path_rgb,path_ir, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        mosaic_size = max(img_size) if isinstance(img_size, (list, tuple)) else img_size
        ...
        try:
            cache_rgb, exists_rgb = np.load(cache_rgb_path, allow_pickle=True).item(), True  # load
        ...
        try:
            cache_ir, exists_ir = np.load(cache_ir_path, allow_pickle=True).item(), True  # load
...
def load_image_rgb_ir(self, i):
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        r = max(self.img_size) / max(h0, w0) if isinstance(self.img_size, (list, tuple)) else self.img_size / max(h0, w0)
...
def load_mosaic(self, index):
    ...
    labels4, segments4 = [], []
    s = max(self.img_size) if isinstance(self.img_size, (list, tuple)) else self.img_size
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                for label in labels:
                    ...
                    l_.append(np.concatenate(([cls_id], np.array(label[:8], dtype=np.float32))))
