...
"""Train the AFDNet baseline reproduction on RGBT-3M fire/person data."""
...
from baselines.AFDNet.model import AFDNetDualStreamDetectionModel, AFDNetYOLO  # noqa: E402
from ultralytics.utils import yaml_load, yaml_save  # noqa: E402


def parse_args():
    """Parse AFDNet baseline training arguments."""
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--cfg",
        type=str,
        default=str(ROOT / "baselines" / "AFDNet" / "cfg" / "AFDNet-y11n-fire-person.yaml"),
        help="AFDNet baseline model YAML.",
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument("--name", type=str, default="train_fp_AFDNet_y11n_seed0_cls01_noaug")
    parser.add_argument("--cls", type=float, default=0.1)
    parser.add_argument("--save_period", type=int, default=-1)
    parser.add_argument("--no_amp", action="store_true")
    parser.add_argument("--enable_aux_head", action="store_true", help="Enable the repo's optional P3 RGB/IR aux heads.")
...
def _copy_training_sources(trainer, train_script_path, cfg_path):
    ...
    for path in (
        train_script_path,
        cfg_path,
        ROOT / "baselines" / "AFDNet" / "model.py",
        ROOT / "baselines" / "AFDNet" / "modules.py",
    ):
        try:
...
def main():
    ...
    if not args.augment:
        ...
        print("[augment] Disabled for AFDNet baseline reproduction.")
