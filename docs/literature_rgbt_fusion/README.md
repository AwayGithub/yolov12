...
| 2   | Edge-Aware Guidance Fusion Network for RGB-Thermal Scene Parsing (EGFNet)                                       | AAAI              | CCF-A 会议 | 2022 | RGB-T semantic segmentation / scene parsing            | <https://github.com/ShaohuaDong2021/EGFNet>              | `pdfs/2022_AAAI_EGFNet.pdf`  | multimodal fusion module, edge-aware guidance, global/semantic information module     | 直接支撑“边界/浅层细节 + 高层语义共同约束”的设计；对 smoke 边界、fire 热源轮廓和 P3 aux 叙事有价值 |
| 3   | SemanticRT: A Large-Scale Dataset and Method for Robust Semantic Segmentation in Multispectral Images (ECM)      | ACM MM            | CCF-A 会议 | 2023 | RGB-T semantic segmentation                            | <https://github.com/jiwei0921/SemanticRT>                | ACM Cloudflare，未下载          | explicit complement modeling, cross-modal feature encoding, robust fusion             | 其“显式互补建模”比简单融合更贴近当前结论，可用于论证 RGB/T 需要保留模态特异信息再融合                 |
| 4   | Multispectral Video Semantic Segmentation: A Benchmark Dataset and Baseline (MVSS / MVNet)                      | CVPR              | CCF-A 会议 | 2023 | RGB-T video semantic segmentation                      | <https://github.com/jiwei0921/MVSS-Baseline>             | `pdfs/2023_CVPR_MVSS_MVNet.pdf` | multispectral-temporal context, MVFuse, embedding regularization, cascaded decoder    | 虽是视频分割，但 MVFuse 的“跨帧/跨模态语义上下文注入”可作为高层语义引导多尺度校准的 A 会支撑         |
| 5   | OpenRSS: Open-Vocabulary RGB-Thermal Semantic Segmentation                                                       | ECCV              | CCF-B 会议 | 2024 | RGB-T semantic segmentation                            | <https://github.com/SXDR/OpenRSS>                        | `pdfs/2024_ECCV_OpenRSS.pdf` | thermal prompt, dynamic LoRA, SAM/CLIP adaptation                                     | 适合借鉴“轻量 adapter/提示注入”；说明不一定要改大 backbone，也可以用参数高效的方式注入热红外信息        |
| 6   | CMX: Cross-Modal Fusion for RGB-X Semantic Segmentation with Transformers                                        | IEEE TITS / arXiv | CCF-B 期刊 | 2023 | RGB-X semantic segmentation, includes RGB-T            | <https://github.com/huaaaliu/RGBX_Semantic_Segmentation> | `pdfs/2023_TITS_CMX.pdf`     | Cross-Modal Feature Rectification, Feature Fusion Module, cross-attention             | 非常适合当前 SGMC：先 rectification，再 fusion；可支撑“P5 生成校准权重，残差修正 P3/P4/P5” |
| 7   | XMSNet: Towards Cross-Modal Semantic Understanding for RGB-D and RGB-T Semantic Segmentation                     | ACM MM            | CCF-A 会议 | 2023 | RGB-D/RGB-T semantic segmentation                      | 代码需继续复核                                                  | `pdfs/2023_ACMMM_XMSNet.pdf` | cross-modal semantic understanding, modality interaction                              | 可借鉴分割任务中的语义一致性监督，适合支持 P3 aux 的语义约束叙事                              |
| 8   | Samba: A Unified Mamba-based Framework for General Salient Object Detection                                      | CVPR Highlight    | CCF-A 会议 | 2025 | RGB/RGB-D/RGB-T salient object detection               | <https://github.com/Jia-hao999/Samba>                    | `pdfs/2025_CVPR_Samba.pdf`   | Mamba, spatial neighboring scanning, context-aware upsampling                         | 不是双分支检测主线，但可为 neck/head 的轻量全局上下文模块提供参考                            |
| 9   | Saliency Prototype for RGB-D and RGB-T Salient Object Detection (SPNet)                                          | ACM MM            | CCF-A 会议 | 2023 | RGB-D/RGB-T SOD                                        | <https://github.com/ZZ2490/SPNet>                        | ACM PDF 未下载                  | saliency prototype, auxiliary modality quality weighting, semantic enhancement        | “先估计辅助模态质量，再融合”很适合烟火场景；可迁移为类别/区域级可靠性门控                            |
...
| TIRDet: Mono-Modality Thermal InfraRed Object Detection Based on Prior Thermal-To-Visible Translation | ACM MM | CCF-A 会议 | 2023 | <https://github.com/zeyuwang-zju/TIRDet> | ACM Cloudflare，未下载 | prior thermal-to-visible translation, latent T2V code, attention-based cross-modality aggregation | 不是严格双输入 RGB-T 检测，但其 CMA 模块说明“可见光语义先验可通过注意力注入热红外检测特征”，适合作为红外分支增强/伪可见语义的补充参考 |

## 四、候选但暂不作为主推

| 论文                                                                         | 来源       | 等级       |   年份 | 代码状态      | 原因                                                               |
| -------------------------------------------------------------------------- | -------- | -------- | ---: | --------- | ---------------------------------------------------------------- |
| Learning a Dynamic Cross-Modal Network for Multispectral Pedestrian Detection (DCMNet) | ACM MM   | CCF-A 会议 | 2022 | 未找到官方公开代码 | 检测任务非常直接，local dynamic convolution + non-local latent node aggregation 与深层跨模态交互高度相关；但未满足“有代码仓库”要求，暂不纳入核心表 |
| Quality-Aware RGBT Tracking via Supervised Reliability Learning and Weighted Residual Guidance (QAT) | ACM MM | CCF-A 会议 | 2023 | 未找到官方公开代码 | reliability/gating 思想与当前“热红外不总是正收益”很相关；任务是 tracking 且代码未确认，作为可靠性门控的补充线索 |
...
7. `pdfs/2022_AAAI_EGFNet.pdf`：重点看 MFM、GIM、SIM 与 edge-aware guidance，适合支撑浅层边界/高层语义联合监督。
8. `pdfs/2023_CVPR_MVSS_MVNet.pdf`：重点看 MVFuse 和 embedding regularization，适合支撑“高层语义上下文指导多尺度特征校准”。
9. SemanticRT / ECM 论文页和代码：重点看 explicit complement modeling，可服务“先保留模态特异信息，再做鲁棒融合”的叙事。
10. `pdfs/2024_TGRS_C2Former.pdf`：重点看 ICA + AFS，和 BidirLiCMA\@P5 直接相关。
11. `pdfs/2022_ECCV_TSRA_TSFADet.pdf`：重点看 feature-level alignment 与 modality selection，适合补充跨模态错位动机。
12. `pdfs/2023_TITS_CMX.pdf`：重点看 feature rectification，可服务 SGMC。
13. `pdfs/2025_TNNLS_TFDet.pdf`：重点看 target-aware fusion 和 false-positive suppression。
...
| 类别                      | 可引用论文                                                                                    | 可支撑的观点                                                 |
| ----------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| RGB/IR 检测特征融合           | CMM, WaveMamba, M2D-LIF, PCDF, DAMSDet, CALNet, ProbEn, C²Former, ICAFusion, TFDet, PFGF, TIRDet | 检测任务需要特征级跨模态交互，但不能简单强融合                                |
| 模态冲突 / 偏置 / 错位          | CMM, M2D-LIF, CALNet, DAMSDet, AANet, TSRA, AMDANet, SemanticRT                           | RGB/IR 存在统计偏置、语义冲突、空间错位、单模态学习不足和可靠性差异                  |
| 多尺度语义校准                 | DAMSDet, CMX, AMDANet, C²Former, PCDF, WaveMamba, EGFNet, MVSS                            | 高层语义、条件提示或频域结构可以指导多尺度特征，但应采用残差校准和轻量门控                  |
