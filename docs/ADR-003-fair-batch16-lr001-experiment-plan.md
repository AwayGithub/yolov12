...
**Status:** 第一批结构主线（M1/M2/M3）与 plainP2 2×2 正交消融（F1-F4）已完成。公平口径下 DMG v1@P2 未证明相对 plainP2 的正增益；SGMC 仅呈现边界收益。
**Date:** 2026-04-28
...
- §7.4 的 F1-F4 2×2 消融表为一次特殊记录：F1-F3 按当前复查需求采用 `results.csv` 最后一行作为训练结束点对比；F4 因旧 CSV 无分类别字段，采用已复核的 `results1.png` 结果。
...
| -- | ------------------------------------- | --------- | -------- | ------ | ---------------------- | --------------------------- |
| F1 | `train_MF_plainP2_P2345_P4A2C2f_P3noaux` | plain     | A2C2f    | off    | **已完成，CSV last mAP50-95=0.63138** | 原始 plainP2 底座               |
| F2 | `train_MF_plainP2_P2345_P4A2C2f_P3aux`   | plain     | A2C2f    | on     | **已完成，CSV last mAP50-95=0.63481** | 单独评估 aux 在旧 P4 下的收益         |
| F3 | `dual_MF_plainP2_P2345_P4C3k2_P3noaux`   | plain     | C3k2     | off    | **已完成，CSV last mAP50-95=0.63075** | 单独评估 C3k2 在无 aux 下的收益       |
| F4 | `dual_MF_plainP2_P2345_P4C3k2_P3aux`     | plain     | C3k2     | on     | **已完成，CSV last mAP50-95=0.63421；results1 mAP50-95=0.634** | 干净 plainP2 对照；对应旧 Exp-8e 口径 |
...
原因：M1 vs M2 已显示 DMG v1@P2 在公平口径下未确认正增益，继续扩展 DMG v1 变体会优先级偏低。plainP2 的 2×2 正交消融完成后，当前更明确的稳定因素是 P3 aux；P4 C3k2 未证明优于 A2C2f。
...
### 7.4 F1-F4：plainP2 2×2 正交消融

本节按当前可复核口径记录：F1-F3 使用 `results.csv` 最后一行；F4 使用 `results1.png`，因为 F4 是较早完成的实验，CSV 只有总体 `metrics/*` 字段，没有分类别字段。F1 的 CSV 最后一行停在 epoch 197，其余 F2/F3 最后一行为 epoch 200；该差异需要在后续 `results1.png` 复核时留意。

| 编号 | 实验目录 | P4 block | P3 aux | 数据来源 | all P | all R | all mAP50 | all mAP50-95 |
| -- | -- | -- | -- | -- | --: | --: | --: | --: |
| F1 | `train_MF_plainP2_P2345_P4A2C2f_P3noaux` | A2C2f | off | CSV last epoch 197 | 0.92850 | 0.89365 | 0.93870 | 0.63138 |
| F2 | `train_MF_plainP2_P2345_P4A2C2f_P3aux`   | A2C2f | on  | CSV last epoch 200 | 0.92889 | 0.90679 | 0.94096 | **0.63481** |
| F3 | `dual_MF_plainP2_P2345_P4C3k2_P3noaux`   | C3k2  | off | CSV last epoch 200 | 0.92986 | 0.89502 | 0.93661 | 0.63075 |
| F4 | `dual_MF_plainP2_P2345_P4C3k2_P3aux`     | C3k2  | on  | `results1.png` | 0.930 | 0.906 | 0.941 | 0.634 |

#### F1 分类别结果（CSV last row）
...
| 对比 | Δ all mAP50-95 | 判断 |
| -- | --: | -- |
| F2 - F1（A2C2f 下加入 P3 aux） | **+0.00343** | P3 aux 在 A2C2f P4 下稳定增益，主要提升 recall、smoke 与 fire |
| F4 - F3（C3k2 下加入 P3 aux） | **约 +0.00325** | P3 aux 在 C3k2 P4 下同样带来增益，说明 aux 是当前 2×2 中最明确的有效因素 |
| F3 - F1（无 aux 下 C3k2 替换 A2C2f） | **-0.00063** | C3k2 在无 aux 时未带来总体收益 |
| F4 - F2（有 aux 下 C3k2 替换 A2C2f） | **约 -0.00081** | C3k2 在有 aux 时也未超过 A2C2f；差距很小，可视为基本持平 |

阶段结论：在当前可复核口径下，F2 是 2×2 中最高点（0.63481），F4 非常接近（0.634）。最稳定的结构因素是 P3 aux：无论 P4 为 A2C2f 还是 C3k2，加入 aux 都带来约 +0.0033～0.0034 的 all mAP50-95。P4 C3k2 没有证明优于 A2C2f，更多是与 A2C2f 持平。若后续写论文，P3 aux 可以作为更稳的消融结论；P4 C3k2 应谨慎表述为“保持性能并调整结构/去噪倾向”，不宜单独宣称显著提升。
...
阶段结论：公平口径下，M2 plainP2 与 M1 DMG v1 的 all mAP50-95 同为 0.634，且 smoke/fire 的 mAP50-95 还分别低于 plainP2 0.002/0.001。DMG v1@P2 没有证明出稳定正作用，不能再作为“明确提升精度”的主创新点。结合 F1-F4 的 2×2 消融，plainP2 下 A2C2f@P4 与 C3k2@P4 基本持平，P3 aux 是更明确的有效因素。后续叙事应转向 P3 aux 的稳定辅助监督作用，或继续验证 SGMC 这类高层语义校准是否能提供第二个边界有效点。SGMC 当前仅带来 all +0.001，主要改善 fire（相对 M2 +0.006），不足以直接声明为强第二创新点。
...
6. F1-F4 已显示 P3 aux 是更稳定的有效因素；P4 C3k2 未证明优于 A2C2f，因此后续不再将 P4 block 作为明确最优结论表述。
