...
**Status:** Exp-7 全部完成。Exp-7b/7d 数值并列最优（mAP50-95=0.634），Exp-4c 仍为最优性价比方案。DMGFusion@P2 全面劣于 Concat@P2。
**Date:** 2026-04-04
**Updated:** 2026-04-17
**Deciders:** 研究者本人
...
1. **D = |RGB-IR| 在 P2 层级语义为零**：P2 只经过两层 Conv + C3k2，特征仍是低层纹理。D 编码传感器级差异（噪声/色温）而非语义不一致，sel_net 在拟合噪声。对照：7d 的 Concat 不依赖 D 的语义假设，故不受此影响。

2. **公式双重计数**：alpha=0、beta=1、w≈0.5 初始化时，`fused ≈ 0.5(x+y) + 0.5(x+y) = x+y`（初始化为 2× 均值），两条路径梯度干扰训练效率。

3. **alpha 无约束增长**：alpha 从 0 起步但无上限，训练初期若梯度推高 alpha，`(1+alpha*S)` 在 P2 高差异区域（多为传感器噪声）放大特征，破坏后续 BN 统计量并向 FPN 传播。

4. **P2 检测头梯度为次要因素**：7a vs 7c 对比（0.628 vs 0.625）显示 P2 head 的额外梯度反而略微缓解了 FPN 污染（差值仅 0.003），主要损害来自 DMGFusion 特征本身。
