...
本文按“先结论、再证据、后复盘”的顺序组织：

1. **快速结论**：直接回答本阶段采用哪一版、为什么。
2. **项目与模块背景**：解释双分支、CMG/CMA、DMGFusion、aux head 的设计语义。
3. **数据与实验总览**：保留全量指标、分类别关键结果、完整分类别原始表。
4. **阶段复盘**：按 Exp-0~4c、Exp-7、Exp-8 三条时间线解释每个结论如何形成。
5. **最终移交与附录**：冻结配置、遗留问题、checkpoint 对照。
...
| 配置 | mAP50 | mAP50-95 | smoke | fire | person | 参数量 | GFLOPs |
|------|------:|---------:|------:|-----:|-------:|-------:|------:|
| Exp-8d | 0.942 | **0.644** | 0.741 | **0.599** | **0.593** | **4.37M** | 31.56 |

Exp-8d 的结构为：**DMGFusion v1 @P2 + P4 C3k2 + P3 aux head + 去除 CMG/CMA + P2/P3/P4/P5 四尺度检测头**。

本阶段最重要的判断有三条：

1. **双分支中期融合是必要前提。** Exp-1 相对 early fusion 的 mAP50-95 从 0.605 提升到 0.623，fire/person 受益最明显。
2. **CMG/CMA 是阶段 I 的诊断性补偿模块，不是最终主干。** 在 aux head 与 P4 C3k2 修复后，去掉 CMG/CMA 的 Exp-8d 反而超过 Exp-8a，且参数量从 5.55M 降至 4.37M。
3. **DMGFusion v1 仍是当前最有效的 P2 融合算子。** 干净基线下三方消融为 plain 0.639 < v2 0.641 < v1 0.644，说明 v1 的差异放大机制仍有独立增益。

**移交到 ADR-002 的问题：** 不再继续堆叠 CMG/CMA；后续应在 Exp-8d 干净基线之上，针对 DMGFusion v1 做小步、可归因的机制改造。
...
### 3.3 DMGFusion v1（P2 融合模块）

差异引导融合：`fused = (1 + α·S)·(w_rgb·R + w_ir·I) + β·(R+I)/2`，其中 `D=|R-I|`，`S=σ(diff_enc(D))`，`[w_rgb,w_ir]=softmax(sel([R;I;D]))`。初始 α=0、β=1。

**设计缺陷（Exp-7 特征可视化证实，见 §7.2）：** β 残差吞噬门控；D 编码尺度差而非语义分歧；softmax 零和约束引发目标处"双低洞"；BN 导致输出幅值塌缩；S 整图饱和；W 无通道维。
...
### 4.2 各尺度对小目标的覆盖（某 stride 下 <2 格子比例）
...
| Exp-8c | Exp-8a，P2 改 **plain** | 0.938 | 0.635 | 0.932 | 0.897 | 5.54M | 30.57 | 17.17 | 完成 |
| **Exp-8d** ★ | **Exp-8a，去 CMG/CMA**（主方案） | **0.942** | **0.644** | **0.930** | **0.903** | **4.37M** | **31.56** | **12.58** | **阶段 III SOTA** |
...
| Exp-8c | 0.635 | 0.735 | 0.589 | 0.579 |
| **Exp-8d** ★ | **0.644** | **0.741** | **0.599** | **0.593** |
| Exp-8e | 0.639 | 0.739 | 0.594 | 0.585 |
| Exp-8f | 0.641 | 0.740 | 0.595 | 0.589 |

### 5.3 完整分类别原始结果表

本节保留 `docs/ADR-old_4.md` 中迁移来的逐类别原始结果表，并补充 Exp-8e/8f 的完整 val 输出。该节用于溯源与复核；阅读主线时优先看 §5.1/§5.2 的总表和关键分类别摘要即可。
...
**Exp-4a（CMG@P4P5 + 单向 CMA@P4P5 RGB<-IR，4.94M，15.27ms）：**
...
**Exp-4b-noCMG（双向 CMA@P4P5，无 CMG，5.34M，16.09ms）：**
...
**Exp-4b（CMG@P4P5 + 双向 CMA@P4P5，5.49M，17.55ms）：**
...
**Exp-4c ★（CMG@P4P5 + 双向 CMA@P5，5.23M，16.05ms）：**
...
**Exp-4d（CMG@P4P5 + 双向 CMA@P4，4.47M，16.68ms）：**
...
**Exp-7a ⚠（DMGFusion@P2 + Exp-4c + head P2P3P4P5，5.32M，31.07G，21.9ms）：**
...
**Exp-8a（Step 1 + Step 2：P4 C3k2 + P3 aux + DMGFusion v1@P2，3366 images / 9227 instances）：**
...
**Exp-8b（Exp-8a + DMGFusion v2，3366 images / 9227 instances）：**
...
**Exp-8e（去 CMG/CMA + P2 plain + aux + C3k2，3366 images / 9227 instances）：**

| 类别 | Images | Instances | P | R | mAP50 | mAP50-95 |
|------|-------:|----------:|------:|------:|------:|---------:|
| all | 3366 | 9227 | 0.937 | 0.902 | 0.943 | **0.639** |
| smoke | 2398 | 4086 | 0.953 | 0.879 | 0.940 | 0.739 |
| fire | 2606 | 3401 | 0.940 | 0.918 | 0.951 | 0.594 |
| person | 1243 | 1740 | 0.918 | 0.908 | 0.937 | 0.585 |

**Exp-8f（去 CMG/CMA + P2 DMG v2 + aux + C3k2，3366 images / 9227 instances）：**

| 类别 | Images | Instances | P | R | mAP50 | mAP50-95 |
|------|-------:|----------:|------:|------:|------:|---------:|
| all | 3366 | 9227 | 0.929 | 0.902 | 0.940 | **0.641** |
| smoke | 2398 | 4086 | 0.949 | 0.881 | 0.941 | 0.740 |
| fire | 2606 | 3401 | 0.930 | 0.912 | 0.941 | 0.595 |
| person | 1243 | 1740 | 0.908 | 0.914 | 0.939 | 0.589 |
...
## 六、阶段 I 复盘：双分支与 CMG/CMA（Exp-0~4c）

### 6.1 双分支中期融合是核心贡献（Exp-0→Exp-1）
...
### 6.5 CMG+CMA@P5 对 person 的阶段性协同（后在 §8 被修正）
...
> **⚠️ §8 修正：** 阶段 III 发现 aux head + C3k2 接入后，CMG/CMA 的贡献被同源更根本的修复吸收。此处协同效应的真正来源是"RGB 独立梯度缺失时的次优补偿"。详见 §8.3。

### 6.6 单向 CMA 只提高 Recall、不提升定位
...
## 七、阶段 II 诊断：DMGFusion@P2 首次失败（Exp-7）

### 7.1 实验层面：Exp-7 的 2×2 析因设计
...
### 7.2 特征可视化诊断（video10_frame_01154）
...
| 3 | 融合模块重构 | DMGFusion v2（见 §3.4） | `block.py`, `tasks.py`, YAML |
...
### 8.3 CMG/CMA 冗余假说（Exp-8a vs Exp-8d）
...
**逐类验证：** person 提升最大（+1.2% from 0.581→0.593），而 person 原本是 §6.5 认为"最依赖 CMG+CMA 协同"的类。此结果**修正 §6.5**：真正起作用的是 RGB 独立梯度本身，CMG/CMA 仅是缺失该梯度时的次优补偿。

### 8.4 DMG v1 / v2 / plain 三方消融（干净基线 Exp-8d/8e/8f）
...
| **Exp-8d ★** | **DMG v1** | **0.644** | **0.741** | **0.599** | **0.593** | **+0.005** |

**三点证据链：**

1. **单调排序 plain < v2 < v1**，间距均 >0.3% 判定阈值，排除噪声。三类 mAP50-95 方向一致（smoke 持平；fire 与 person 均随档位上升）。
2. **v2 的有限收益**（+0.002）与 §8.5.3 viz 结论吻合：v2 六项改动中仅 `sigmoid + IN(D)` 两项小幅正作用，删除 α·S 等于移除 v1 的主工作机制（Exp-8a α=+0.999 饱和）。v2 在干净基线下凭借 sigmoid/IN 细节弥补一部分，但无法弥补核心机制缺失。
3. **v1 的独立增益**（+0.005）在干净基线上仍被保留，结合 §8.5.2 的跨基线 α 符号翻转证据，**DMG v1 作为融合算子的有效性获得两条独立证据链支持**。

### 8.5 Exp-8 详细诊断

#### 8.5.1 Exp-8a 详细结果（val 3366 imgs / 9227 instances）
...
#### 8.5.2 DMGFusion v1 @P2 收敛标量（跨基线对比）
...
| Class | Instances | P | R | mAP50 | mAP50-95 |
|-------|----------:|------:|------:|------:|------:|
| **all**  | 9227 | 0.932 | 0.897 | 0.938 | **0.635** |
| smoke    | 4086 | 0.956 | 0.876 | 0.940 | 0.735 |
| fire     | 3401 | 0.928 | 0.912 | 0.942 | 0.589 |
| person   | 1740 | 0.912 | 0.904 | 0.931 | 0.579 |

**模型规模：** 5,540,616 params / 30.57 GFLOPs / 869 layers  
**推理：** preprocess 0.58 + inference 17.17 + postprocess 1.68 ≈ 19.43 ms/image
...
- Exp-8a → Exp-8d：mAP50-95 +0.006，person +1.2%，Params −21%（5.55M → 4.37M）—— CMG/CMA 冗余（§8.3）。
- Exp-8d vs Exp-8e/8f：DMG v1 相对 plain 独立 +0.005，相对 v2 +0.003 —— 三方消融链完整（§8.4）。
...
1. **DMG v1 内部存在明显可优化点。** §8.5.3 viz 已指出：β 残差与 α·S 调制路径在 Exp-8a 已进入非平凡工作点，Exp-8d 进一步演化为 α=+2.420 / β=−0.202 的"差分放大 + 共模抑制"形态（§8.5.2）——核心机制确实在工作且仍未饱和上限。但 softmax 零和、W 无通道维、BN 幅值塌缩等缺陷未在 Exp-8d 中解决。v1→v2 的失败在于重构策略错误（大改而非定向微调）。下阶段应在 v1 基础上只替换 softmax→sigmoid + 前置 IN(D)，保留 α·S+β。
...
- 推理对比时确保同一硬件（§5.1 表中 Exp-8 行与其他行硬件一致）
