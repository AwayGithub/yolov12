...
| 实验 | 输入配置 | mAP50 | mAP50-95 | Precision | Recall | 推理耗时(ms) | 状态 |
|------|---------|-------|----------|-----------|--------|------------|------|
| **Exp-0** | 单分支 6ch early fusion | 0.919 | 0.605 | 0.914 | 0.877 | 4.51 | 已完成 |
| **Exp-1** | **对称双分支 concat-only (无 CMG)** | **0.933** | **0.623** | **0.924** | **0.893** | 6.96 | **已完成** |
| Exp-5 | RGB-only 3ch | 0.907 | 0.583 | 0.920 | 0.854 | 4.08 | 已完成 |
| Exp-6 | IR-only 3ch | 0.887 | 0.564 | 0.888 | 0.831 | 4.33 | 已完成 |
...
### 2. CMG 的门控机制恰好解决 smoke 类的 IR 噪声问题

实验显示 IR 对 smoke 检测引入了误检。这说明简单的 concat 融合（early fusion）缺乏"选择性"。CMG 的 Sigmoid 门控天然具备这个能力——当 IR 对某类目标无用时，门控权重会学习到接近 0，自动抑制 IR 信号的干扰。这是 **CMG 相对于简单 concat 的核心卖点**，实验数据已经预先提供了论据。

### 3. 增益天花板的预判（已部分验证）

~~Early fusion（最粗暴的融合）已经拿到了 +1.2 mAP50。~~
- ~~合理的预期目标：mAP50 ≥ 0.925, mAP50-95 ≥ 0.62~~ **✓ Exp-1 已达 0.933/0.623**
- ~~如果 fire 类的 mAP50-95 能从 0.545 提到 0.56+~~ **✓ Exp-1 fire mAP50-95=0.575 (+3.0)**
- **Exp-1 仅用 concat（无 CMG）就已超过预期**，说明双分支独立特征提取是增益的主要来源
- **新预期：** 加上 CMG 后（Exp-2），mAP50 能否达到 0.94+？CMG 对 smoke 类的 Precision 提升值得关注

### 4. 推理耗时约束（已实测）

~~双分支架构参数量增加 ~50%，推理耗时可能增加到 5.5-6ms。~~ **实测 6.96ms**，比预估略高，但仍可接受（~143 FPS）。主要原因是双 backbone 需要串行执行两遍完整的特征提取。后续非对称 backbone（IR 用更轻的 C3k2）可能将推理耗时压回 6ms 以下。
...
| Exp-2 | 对称双分支 + CMG@P4P5 | 验证跨模态门控的增益 | **下一步** | — | — |
| Exp-3 | 非对称双分支 + CMG@P4P5 (RGB:A2C2f, IR:C3k2) | 验证模态特异化 backbone 的增益 | 待做 | — | — |
| Exp-4 | 非对称双分支 + CMG@P3P4P5 | 验证更多交互点是否有益 | 待做 | — | — |
| Exp-5 | 单分支 RGB-only (ch=3) | 单模态 baseline | **已完成** | 0.907 | 0.583 |
| Exp-6 | 单分支 IR-only (ch=3) | 单模态 baseline | **已完成** | 0.887 | 0.564 |

### 下一步实验计划

**优先级 1: Exp-2 — 对称双分支 + CMG@P4P5**

- **目的：** 在 Exp-1 已验证双分支架构增益的基础上，验证 CrossModalGating 能否进一步提升精度
- **操作：** 修改 `yolov12-dual.yaml` 中 `cmg_stages: []` → `cmg_stages: [p4, p5]`，其他训练超参与 Exp-1 保持一致
- **预期：** mAP50 ≥ 0.935, mAP50-95 ≥ 0.63；重点观察 smoke 类 Precision 是否提升（CMG 抑制 IR 噪声）和 fire 类 mAP50-95 是否进一步提高（CMG 放大 IR 贡献）
- **关键判断：** 如果 CMG 增益 < 0.5 mAP50，说明简单 concat 已足够，论文可重点论证架构设计而非融合模块

**优先级 2: Exp-3 — 非对称双分支 + CMG@P4P5**

- **目的：** 验证模态特异化 backbone（RGB 用 A2C2f，IR 用 C3k2）是否进一步提升
- **操作：** 新建 `yolov12-dual-asym.yaml`，IR 分支 backbone 将 A2C2f 替换为 C3k2；CMG 配置与 Exp-2 相同
- **前置条件：** Exp-2 完成后再做，以隔离非对称 backbone 的增益
- **预期：** IR 分支参数量减少，推理略快；fire 类 mAP50-95 可能有小幅提升

**优先级 3: Exp-4 — 非对称双分支 + CMG@P3P4P5**

- **目的：** 验证在 P3 也做跨模态交互是否有益（P3 分辨率大，计算量增加）
- **前置条件：** Exp-3 完成后再做
...
### 后续需要重新审视的事
- 如果 CMG 增益不明显，考虑替换为交叉注意力或 PPAS 式 patch 选择
...
7. [x] ~~Exp-1: 对称双分支 concat-only~~ — **已完成，mAP50=0.933 (+1.4), mAP50-95=0.623 (+1.8)，显著超越 early fusion**
8. [ ] **Exp-2: 对称双分支 + CMG@P4P5** — 修改 YAML `cmg_stages: [p4, p5]` 即可开始训练
9. [ ] Exp-3: 非对称双分支 + CMG — 需新建 `yolov12-dual-asym.yaml`（IR 分支 A2C2f→C3k2）
10. [ ] 重点观察 fire 类 mAP50-95 和 smoke 类 Precision 的变化趋势
