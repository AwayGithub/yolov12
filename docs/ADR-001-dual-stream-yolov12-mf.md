...
| **Exp-4a** | **对称双分支 + CMG@P4P5 + CMA@P4P5** | **验证空间级跨模态查询（RGB Q=RGB KV=IR）** | **进行中** | — | — |
| Exp-5 | RGB-only 3ch | 单模态基准 | 完成 | 0.907 | 0.583 |
| Exp-6 | IR-only 3ch | 单模态基准 | 完成 | 0.887 | 0.564 |

**Exp-4a 判断标准：**
- mAP50 ≥ 0.936（超过 Exp-2）且推理 < 8ms → 方案可行，作为论文主要贡献
- 重点观察 fire 类 mAP50-95（IR 主导类，最能体现空间级跨模态查询的价值），目标突破 0.585
- smoke Recall 是否恢复到 0.91+（RGB 的 Q 应能"学会忽略"IR 对 smoke 无响应的区域）
