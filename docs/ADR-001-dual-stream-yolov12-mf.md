...
## 已有实验数据（2026-03-18 周报）

以下为 YOLOv12n + RGBT-3M 数据集，输入分辨率 480×640 的实验结果：

| 实验 | 输入配置 | mAP50 | mAP50-95 | Precision | Recall | 参数量 | GFLOPs | 推理耗时(ms) |
|------|---------|-------|----------|-----------|--------|--------|--------|------------|
| **Exp-0** | RGB+IR 6ch early fusion | **0.919** | **0.605** | 0.914 | **0.877** | 2.51M | 5.91 | 4.51 |
| **Exp-5** | RGB-only 3ch | 0.907 | 0.583 | **0.920** | 0.854 | 2.51M | 5.82 | 4.08 |
| **Exp-6** | IR-only 3ch | 0.887 | 0.564 | 0.888 | 0.831 | 2.51M | 5.82 | 4.33 |
...
- **需要超越的基准线: mAP50 > 0.919, mAP50-95 > 0.605**
...
Early fusion（最粗暴的融合）已经拿到了 +1.2 mAP50。中期融合理论上应该做得更好，但：
- 如果双分支 mid-fusion 只是持平 0.919，说明架构增益被参数量增加稀释了
- **合理的预期目标：mAP50 ≥ 0.925, mAP50-95 ≥ 0.62**（在 early fusion 基础上再提 0.6-1.5 个点）
- 如果 fire 类的 mAP50-95 能从 0.545 提到 0.56+，就能证明 mid-fusion 确实让 IR 信息被更好利用了

### 4. 推理耗时约束

Early fusion 仅增加 0.43ms。双分支架构参数量增加 ~50%，推理耗时可能增加到 5.5-6ms。在无人机场景下这仍然可接受（>150 FPS），但需要实测确认。
...
|---------|------|------|------|-------|----------|
| Exp-0 | 单分支 6ch early fusion | 基准 (需要超越) | **已完成** | 0.919 | 0.605 |
| Exp-1 | 对称双分支 concat-only (无 CMG) | 验证双分支本身的增益 | 待做 | — | — |
| Exp-2 | 对称双分支 + CMG@P4P5 | 验证跨模态门控的增益 | 待做 | — | — |
...
**实验优先级调整：** 基于模态特异性的发现，将非对称双分支（Exp-3）提前到与对称双分支（Exp-2）并行或紧随其后，而非作为后续优化。
...
3. [ ] 实现 `CrossModalGating` 模块并注册到 `__init__.py`
4. [ ] 实现 `DualStreamDetectionModel` 类（支持对称/非对称两种模式）
5. [ ] 创建 `yolov12-dual.yaml` 和 `yolov12-dual-ir.yaml`（IR 分支用 C3k2 替代 A2C2f）
6. [ ] 修改 `get_model()` 支持双分支
7. [ ] Exp-1: 对称双分支 concat-only → 验证双分支架构本身的增益
8. [ ] Exp-2: 对称双分支 + CMG → 验证门控交互的增益
9. [ ] Exp-3: 非对称双分支 + CMG → 验证模态特异化的增益
