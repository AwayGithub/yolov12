...
class BaseModel(nn.Module):
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        if m == self.model[0]:
            LOGGER.info(f"{'time (ms)':>10s} {'GFLOPs':>14s} {'params':>10s}  module")
        LOGGER.info(f"{dt[-1]:10.2f} {flops:14.6f} {m.np:10.0f}  {m.type}")
        if c:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=None, nc=None, verbose=True):  # model, input channels, number of classes
...
class DualStreamDetectionModel(DetectionModel):
    ...
    def __init__(self, cfg="yolov12-dual.yaml", ch=None, nc=None, verbose=True):
        ...
        for stage_name in self.parallel_cross_a2c2f_stages:
            ...
            self._parallel_cross_layer_to_stage[layer_idx] = stage_name
    ...
    def _get_layer_out_channels(layer):
        """获取层的输出通道数。"""
        if hasattr(layer, "cv2_rgb"):  # DualParallelCrossA2C2f
    ...
    def _forward_both_backbones(self, x_rgb, x_ir):
        ...
        for m_rgb, m_ir in zip(self.backbone_rgb, self.backbone_ir):
            ...
            if layer_idx in self._parallel_cross_layer_to_stage:
    ...
    def adapter_debug_state(self):
        ...
        layer_to_stage = getattr(self, "_parallel_cross_layer_to_stage", {})
